...
string DHT::getDefinition(string w) {
	...
	if (returned == false) return "Uh Oh! Word doesn't exist in our dictionary :(";
}
...
void UI::process(void) {
	while (!exit_program) {
		...
		case SCREEN_MAIN:
			...
			cout << " \n\n\t\t\t\t\t\t----------------------------" << endl;
			cout << "\t\t\t\t\t\t  SMART ENGLISH DICTIONARY" << endl;
			cout << "\t\t\t\t\t\t----------------------------\n" << endl << endl;
		...
		case SCREEN_SEARCH:
			...
			if (!stop_taking_str_input) {
				...
				else if (key_input != 0 && key_input != 13 && key_input != KEY_BACKSPACE && validEntry()) str_input.push_back(key_input); // Remove character when BACKSPACE is pressed
